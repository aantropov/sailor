...
RHI::MaterialPtr VulkanGraphicsDriver::CreateMaterial(const RHI::VertexDescriptionPtr& vertexDescription, const RHI::RenderState& renderState, const Sailor::ShaderSetPtr& shader)
{
	SAILOR_PROFILE_FUNCTION();
	...
	res->m_vulkan.m_pipeline = VulkanPipelinePtr::Make(device,
		...
		device->GetPipelineBuilder()->BuildPipeline(vertexDescription, renderState),
		0);
