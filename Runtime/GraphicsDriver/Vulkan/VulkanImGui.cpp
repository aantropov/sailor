...
namespace Sailor::GraphicsDriver::Vulkan
{
	...
	static void CreateOrResizeBuffer(RHI::RHIBufferPtr& buffer, size_t newSize, VkBufferUsageFlagBits usage)
	{
		SAILOR_PROFILE_FUNCTION();

		ImGui_Data* bd = ImGui_GetBackendData();
		ImGui_InitInfo* v = &bd->VulkanInitInfo;
		VkDeviceSize vertex_buffer_size_aligned = ((newSize - 1) / bd->BufferMemoryAlignment + 1) * bd->BufferMemoryAlignment;
	...
	static void ImGui_SetupRenderState(ImDrawData* drawData, RHI::RHICommandListPtr cmdList, ImGuiH_FrameRenderBuffers* rb, int width, int height)
	{
		SAILOR_PROFILE_FUNCTION();
	...
	// Render function
	void ImGui_RenderDrawData(ImDrawData* drawData, RHI::RHICommandListPtr transferCmdList, RHI::RHICommandListPtr drawCmdList, VulkanGraphicsPipelinePtr pipeline)
	{
		SAILOR_PROFILE_FUNCTION();
	...
	bool    ImGui_Init(ImGui_InitInfo* info)
	{
		...
		io.BackendRendererName = "imgui_impl_sailor_vulkan";
