...
public static class HierarchyProjectionBuilder
{
    ...
    static HierarchyProjectionNode BuildNode(HierarchySourceItem item, IReadOnlyDictionary<string, HierarchySourceItem[]> childrenLookup, ISet<string> expandedIds, string? selectedId)
    {
        var children = childrenLookup.TryGetValue(item.Id, out var childItems)
            ? childItems.Select(child => BuildNode(child, childrenLookup, expandedIds, selectedId)).ToArray()
